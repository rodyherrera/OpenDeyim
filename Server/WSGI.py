...
def Application(*Arguments, **KeywordArguments):
    import sys
    sys.argv = ['--wsgi']
    for Keyword in KeywordArguments:
        ParsedKeyword = Keyword.replace('_', '-')
        if isinstance(KeywordArguments[Keyword], bool) and KeywordArguments[Keyword]:
            sys.argv.append('--' + ParsedKeyword)
        else:
            sys.argv.append('--' + ParsedKeyword)
            sys.argv.append(KeywordArguments[Keyword])
    Instance = Main()
    if len(KeywordArguments) == 0:
        return Instance(*Arguments, **KeywordArguments)
    else:
        return Instance